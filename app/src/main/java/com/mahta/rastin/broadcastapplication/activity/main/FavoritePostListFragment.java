...
import com.mahta.rastin.broadcastapplication.adapter.FavoritePostAdapter;
...
public class FavoritePostListFragment extends Fragment {
    public FavoritePostAdapter favoriteAdapter;
    public TextView txtNoPost;
    private RecyclerView rcvPosts;
    public SearchView searchView;
    private FloatingActionButton btnShowAllPost;

    private Integer currentId = null;
    private Integer currentPosition= null;

    public FavoritePostListFragment() {}


    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        final View view = inflater.inflate(R.layout.fragment_favorite_post_list, container, false);
        rcvPosts = view.findViewById(R.id.rcvMedia);
        setupToolbar(view);

        btnShowAllPost = view.findViewById(R.id.btnShowAllPost);
        btnShowAllPost.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((MainActivity)getActivity()).bringPostsFragment();
            }
        });
        txtNoPost = view.findViewById(R.id.txtNoPost);
                ...
                if (G.isNetworkAvailable(getActivity())){
                    ...
                    if (fp != null){
                        if (G.isNetworkAvailable(getActivity())){
                            ...
                            intent.putExtra(Keys.KEY_EXTRA_FLAG,new Post(fp));
                            currentId = fp.getId();
        ...
        rcvPosts.setOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (dy > 0 && btnShowAllPost.getVisibility() == View.VISIBLE) {
                    btnShowAllPost.hide();
                } else if (dy < 0 && btnShowAllPost.getVisibility() != View.VISIBLE) {
                    btnShowAllPost.show();
                }
            }
        });
